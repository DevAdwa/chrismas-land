...
//			new Thread(printer).start();
     ...
     * 设置行间距
     *
     * @param space          行间距单位，每单位0.125mm,[0,127]
     * @param statusListener 打印机状态监听
     */
...
//				new Thread(printerPic).start();
    ...
    public synchronized void printView(View v, printerStatusListener statusListener) {
        try {
            ...
            if (!flag) {
                SimpleDateFormat format = new SimpleDateFormat("dd/MM/y", Locale.getDefault());
                /**((TextView)v.findViewById(R.id.payment_means)).setText(check.getMeanCode());
                 ((TextView)v.findViewById(R.id.order_number)).setText(String.valueOf(check.getOrderNumber()));
                 ((TextView)v.findViewById(R.id.date)).setText(format.format(System.currentTimeMillis()));
                 ((TextView)v.findViewById(R.id.prestation)).setText(check.getOperation());
                 ((TextView)v.findViewById(R.id.client)).setText(String.valueOf(check.getClient()));
                 ((TextView)v.findViewById(R.id.amount)).setText(FormatUtil.formatNumber(check.getAmount()));
                 ((TextView)v.findViewById(R.id.driverNames)).setText(check.getDriver().concat("\n\n"));
                 ((TextView)v.findViewById(R.id.printing_date_time)).setText(SimpleDateFormat.getInstance().format(System.currentTimeMillis()).concat("\n\n\n\n"));**/
...
//				Log.d("zzd", "state: "+state[0]+" : "+state[1]);
     ...
     * @param displayCode   是够显示条码信息
     * @return
     * @throws WriterException
     */
    public static Bitmap createBarCode(Context context, String contents, BarcodeFormat type, int desiredWidth,
