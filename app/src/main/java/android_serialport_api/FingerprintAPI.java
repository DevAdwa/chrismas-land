...
import com.example.fecafootdemo.utils.Constant;
...
public class FingerprintAPI {
	...
	private void sendCommand(byte[] command) {
		...
		SerialPortManager.getInstance().write(getPacket(command, Constant.FINGER_ID));
	}
	public byte[] getPacket(byte[] data, int id) {
		byte[] header = new byte[] { (byte) 0xCA, (byte) 0xDF, 0x00, 0x35 };
		byte[] ed = new byte[] { (byte) 0xE3 };
		byte[] packet = Bytes.concat(header, new byte[] { (byte) data.length }, data,ed);
